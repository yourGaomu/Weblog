...
public class RestAuthenticationEntryPoint implements AuthenticationEntryPoint {
    ...
    public void commence(HttpServletRequest request, HttpServletResponse response, AuthenticationException authException) throws
        ...
        if (authException instanceof InsufficientAuthenticationException) {
            AuthResponseResultUtil.fail(response, HttpStatus.UNAUTHORIZED.value(), R.Faile(ResponseCodeEnum.UNAUTHORIZED));
            return;
        }

        AuthResponseResultUtil.fail(response, HttpStatus.UNAUTHORIZED.value(), R.Faile(authException.getMessage()));
